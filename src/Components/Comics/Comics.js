...
function Comics() {
  ...
  const fetchBooks = async () => {
    try {
      ...
      const cached = localStorage.getItem("comicsBooks");
      ...
      const res = await axios.get(
        "https://www.googleapis.com/books/v1/volumes?q=subject:comics&maxResults=20"
      );

      const formatted = formatBooks(res.data.items, "Comics");

      setBooks(formatted);
      localStorage.setItem("comicsBooks", JSON.stringify(formatted));
